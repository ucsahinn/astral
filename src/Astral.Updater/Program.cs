...
return await AstralUpdater.RunAsync(args, progress);

internal static class AstralUpdater
{
        ...
        UpdateProgressPresenter progress)
    {
        ...
        try
        {
            progress.Report(
                ...
                "Astral'ın kapanması bekleniyor.");
            log.Write("Update helper started.");
            await WaitForAstralToExitAsync(options.ProcessId, log);
            ...
            progress.Report(
                94,
                "Astral yeniden açılıyor",
                "Yeni sürüm başlatılıyor.");
            log.Write("Starting updated Astral.");
    ...
    private static async Task WaitForAstralToExitAsync(
        int processId,
        UpdateLog log)
    {
        try
        {
            ...
            log.Write("Waiting for Astral to exit.");
            ...
            catch (OperationCanceledException)
            {
                throw new TimeoutException(
                    "Astral did not exit before the update timeout.");
            }
        }
        catch (ArgumentException)
        {
            log.Write("Astral process was already closed.");
        }
    }
...
internal sealed class UpdateLog
{
    ...
    {
        var redacted = AstralDiagnostics.RedactForLog(message) ?? string.Empty;
...
internal sealed class UpdateProgressForm : Form
{
    ...
    {
        var redacted = AstralDiagnostics.RedactForLog(text) ?? string.Empty;
