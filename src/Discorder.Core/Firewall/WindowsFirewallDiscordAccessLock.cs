...
public sealed class WindowsFirewallDiscordAccessLock : IDiscordAccessLock
{
    ...
    private static string BuildApplyTunnelScopeScript(bool includeBrowserAccess)
    {
        ...
        return string.Join(Environment.NewLine, [
            ...
            "    )",
            "    function Test-DiscorderBrowserProgram([string]$path) {",
            ...
            "    return @($allCandidates | Where-Object { (Test-DiscorderBrowserProgram $_) -and (Test-Path -LiteralPath $_) } | Sort-Object -Unique)",
            "}",
