...
{
    public const string ProductDataDirectoryName = "Astral";
    public static string LegacyProductDataDirectoryName => string.Concat("Dis", "corder");
    ...
    public AppPaths(string? localAppData = null, string? commonAppData = null)
    {
        ...
        DataDirectory = Path.Combine(root, ProductDataDirectoryName);
        SharedDataDirectory = Path.Combine(sharedRoot, ProductDataDirectoryName);
        LegacyDataDirectory = Path.Combine(root, LegacyProductDataDirectoryName);
        LegacySharedDataDirectory = Path.Combine(
            sharedRoot,
            LegacyProductDataDirectoryName);
        ...
        WireSockInstallMarker = Path.Combine(DataDirectory, "wiresock-installed-by-astral.marker");
        LegacyWireSockInstallMarker = Path.Combine(
            LegacyDataDirectory,
            "wiresock-installed-by-" + LegacyProductDataDirectoryName.ToLowerInvariant() + ".marker");
    ...
    public string LegacySharedDataDirectory { get; }
    ...
    public string WebProxyDirectory { get; }
    ...
    public string LegacyWireSockInstallMarker { get; }
    ...
    public string WebProxyPacFile { get; }

    public string WebProxyStateFile { get; }
    ...
    private static void RejectReparsePointsInExistingAncestors(string path)
    {
        ...
        if (string.IsNullOrWhiteSpace(root))
        {
            throw new InvalidOperationException(
                "Astral veri yolu çözümlenemedi.");
        }
    ...
    private static void RejectReparsePoint(string directory)
    {
        ...
        {
            throw new InvalidOperationException(
                "Astral veri klasörü junction veya symlink olamaz.");
        }
    }
}
