    ...
    public static string GetRequiredSignerThumbprint(string path)
    {
        ...
        if (string.IsNullOrWhiteSpace(thumbprint))
        {
            throw new InvalidDataException(
                $"{Path.GetFileName(path)} imzalı değil. Güvenli güncelleme için Astral yayınları imzalı olmalıdır.");
        }
...
#pragma warning restore SYSLIB0057
    ...
    public static void VerifyFile(string path, string expectedSignerThumbprint)
    {
        ...
        {
            throw new InvalidDataException(
                $"{Path.GetFileName(path)} beklenen Astral imzasıyla eşleşmiyor.");
        }
    }
