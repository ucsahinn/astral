...
namespace Astral.Core.Maintenance;

public sealed class AstralCleanupService
{
    ...
    public async Task CleanProfileAsync(CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        _diagnostics.Warning(
            "cleanup.profileClean",
            "Profil temizliği başladı.",
        ...
        try
        {
            ...
            foreach (var target in targets)
            {
                DeleteAstralDirectory(target, cancellationToken);
            }
        }
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            ...
            _diagnostics.Failure(
                "cleanup.profileClean",
                "Profil temizligi tamamlanamadi.",
                exception);
            throw;
        }
    }
    ...
    {
        if (_diagnostics is AstralDiagnostics diagnostics)
        {
    ...
    private string[] GetCleanupTargets()
    {
        return new[]
        {
            ...
            _paths.LegacyDataDirectory,
            _paths.LegacySharedDataDirectory
        }
    ...
    private void ValidateCleanupTarget(string path)
    {
        if (!_allowNonDefaultDataRoots
            && !IsDefaultKnownDataRoot(path))
        {
            throw new InvalidOperationException(
                "Astral veri klasoru beklenen AppData konumunda degil.");
        }
        ...
        if (!IsKnownDataDirectoryName(directory.Name) || directory.Parent is null)
        {
            throw new InvalidOperationException(
                "Astral veri klasoru beklenen konumda degil.");
        }

        RejectReparsePoints(directory);
    }

    private static bool IsDefaultKnownDataRoot(string path)
    {
        ...
        var expected = new[]
        {
            Path.Combine(localRoot, AppPaths.ProductDataDirectoryName),
            Path.Combine(sharedRoot, AppPaths.ProductDataDirectoryName),
            Path.Combine(localRoot, AppPaths.LegacyProductDataDirectoryName),
            Path.Combine(sharedRoot, AppPaths.LegacyProductDataDirectoryName)
        }
            .Select(NormalizeDirectoryPath);
    ...
    private static bool IsKnownDataDirectoryName(string name)
    {
        return string.Equals(
            ...
            || string.Equals(
                name,
                AppPaths.LegacyProductDataDirectoryName,
                StringComparison.Ordinal);
    }
    ...
    private static void DeleteAstralDirectory(
        string path,
        CancellationToken cancellationToken)
    {
        ...
        if (directory.Exists)
        {
            throw new IOException(
                "Astral veri klasoru temizlenemedi: " +
                directory.FullName);
        }
    }
    ...
    private static void RejectReparsePoints(DirectoryInfo root)
    {
        ...
        while (directories.Count > 0)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Astral veri klasoru junction veya symlink olamaz: " +
                    directory.FullName);
            }
            ...
            foreach (var entry in directory.EnumerateFileSystemInfos())
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Astral veri klasoru junction veya symlink iceremez: " +
                        entry.FullName);
                }
    ...
    private static void ClearReadOnlyAttribute(FileInfo file)
    {
        ...
        {
            throw new InvalidOperationException(
                "Astral veri klasorunde hard link iceren dosya temizlenemez: " +
                file.FullName);
        }
    ...
    private static bool HasMultipleHardLinks(FileInfo file)
    {
        ...
        {
            throw new InvalidOperationException(
                "Astral veri klasorundeki dosya baglanti bilgisi dogrulanamadi: " +
                file.FullName,
                exception);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(
                "Onarim klasoru Astral veri kokunun disinda.");
        }

        DeleteDirectoryWithRetry(
            directory,
            "Astral onarim klasoru temizlenemedi: ",
            cancellationToken);
    }
