...
public partial class MainWindow : Window, IDisposable
{
    ...
    private static readonly Uri ReleaseNotesUri = new(
        "https://github.com/ucsahinn/discorder/releases/tag/v2.1.6");
    ...
    private string GetDetail(TunnelState state)
    {
        return state switch
        {
            TunnelState.Connected => _controller.IncludeBrowserAccess
                ? "Discord uygulaması ve tarayıcı bağlantıya dahil."
                : "Yalnızca Discord uygulaması bağlantıya dahil.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            DiagnosticsStatus.Text =
                $"v{AppUpdateService.FormatVersion(update.LatestVersion)} hazır. Güncelle, bu Discorder klasörünü yeniler.";
            _diagnostics.Info(
                "ui.update.available",
                "Güncelleme bulundu.",
                new Dictionary<string, string?>
