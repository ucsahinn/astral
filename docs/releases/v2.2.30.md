...
- App kapsamı olan hedeflerde `ScopedWebProxy` başarısı tek başına bağlantı başarısı sayılmaz; WireSock handshake veya adapter trafik kanıtı alınmazsa denetleyici `Connected` yerine `Error` durumuna geçer.
...
- UI hedef testi artık HTTP/CONNECT başarısını uygulama kapsamı hazır gibi saymaz; app kapsamı için ayrı profil/WireSock kanıtı beklenir.
