...
- Discord dışı app hedefleri için yeni `targetAppProof.*` tanılama modeli ve Windows hedef uygulama kanıt sağlayıcısı eklendi. Sağlayıcı hedef exe sürecini ve bu sürecin hedef probe IPv4/IPv6 adreslerine owned established TCP bağlantısını doğrularsa bağlantı matrisi devam eder; sağlayıcı yoksa veya eksik hedef dönerse Astral fail-closed kalır.
...
- Disconnect ve restart cleanup akışında WireSock process handle'ı dispose sonrasında okunamaz hale gelirse (`No process is associated with this object`) kapanış fatal hataya düşmez; süreç durumu güvenli helper'larla okunur ve health/events çıktısı korunur.
- İlk scoped WebProxy kanıtı geçtikten sonra oluşan upstream `CONNECT` hataları artık tanılama detaylarında `webProxyRuntime.failuresDetected`, `webProxyRuntime.failureCount` ve `webProxyRuntime.lastFailure` alanlarıyla görünür olur.
...
- `scripts/smoke-live-connect.ps1`, app kapsamlı hedef seçildiğinde `HealthHasRequiredApplicationProof` kapısını zorunlu kılar; Discord dışı app hedeflerinde bu kapı artık yalnız adapter trafik artışına değil `targetAppProof.verified=True` ve boş `targetAppProof.missingTargetIds` kanıtına da bakar.
...
- Core regresyon testleri app+web yanlış bağlı pozitifini, Discord dışı app hedeflerini, hedef-app proof pozitif/negatif kapısını, karışık app hedeflerini, WebProxy runtime upstream tanılamasını ve cleanup/process dispose doğrulama hatalarını kapsar.
