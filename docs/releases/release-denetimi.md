...
Güncel patch hedef `v2.2.26` olarak belirlendi.
...
- v2.2.25, istenen yeni arka plan videosunu SHA-256 doğrulamalı repo-local `Assets/background.mp4` olarak paketledi; release build ve runtime uzak video fallback'i kullanmaz.
- v2.2.26, bu video düzeltmesini korur ve Roblox aktif preset/resource/smoke izlerini kaldırarak 15 hedefli güncel listeyi yayınlar.
...
- v2.2.26, hedef listeli proof kapısı, 15 aktif preset, karttan web açma, yeni arka plan videosu ve Astral VPN ürün yüzeyiyle bu hattı tamamlar.

## Public Release Yüzeyi

| Release | Canlı durum | Asset durumu | Karar | Gerekçe |
| --- | --- | --- | --- | --- |
| `v2.2.26` | Aday | Bekleniyor | Yayına hazırsa oluştur | Yeni arka plan video paketi, Roblox preset temizliği, hedef linkleri, Astral VPN ürün yüzeyi ve hedef listeli proof kapısı. |
...
`v2.2.26` yayınından önce tamamlanması gereken koşullar:

- Proje, updater ve web proxy sürümü `2.2.26` ile aynı.
- `src/Astral.App/app.manifest` kimlik sürümü proje sürümüyle aynı.
- `Astral-2.2.26-win-x64.zip` ve sabit `Astral-win-x64.zip` aynı içeriği gösterir.
- ZIP içinde `Astral.exe`, `Astral.Updater.exe`, `Astral.WebProxy.exe`, update manifest'i ve gerekli runtime dosyaları bulunur.
- ZIP içindeki update manifest sürümü `2.2.26`.
