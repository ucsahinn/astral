...
- Uygulama hedefleri WireSock uyumlu `#@ws:AllowedApps` satırına yazılır.
- Web hedefleri için genel tarayıcı exe'leri `#@ws:AllowedApps` içine yazılmaz; yalnızca `Astral.WebProxy.exe` girer.
- WireSock varsayılan transparent mode ile başlatılır; bu modda sanal adaptör beklemek bağlantı şartı değildir.
...
10. Seçili uygulama exe'leri ve gerekiyorsa `Astral.WebProxy.exe` yeni `#@ws:AllowedApps` satırına yazılır.
...
13. Readiness doğrulaması transparent mode'da sanal adaptörü zorunlu saymaz; WireSock süreci sağlıklı kalmalı, erken çıkış yapmamalı ve hedef kapsamı dar kalmalıdır.
...
Bu tasarımda tarayıcı süreci WireSock kapsamına girmez. Tarayıcı normal kalır; yalnız seçili domain istekleri PAC üzerinden `Astral.WebProxy.exe` sürecine yönlenir. Proxy süreci WireSock `#@ws:AllowedApps` içinde olduğu için yalnız bu seçili web hedefleri tünel yoluna girer.
...
- Transparent mode WireSock süreci sağlıklı kaldığı sürece sanal adaptör üretmeyebilir; `wiresock-adapter-inactive` tek başına hata kanıtı değildir.
