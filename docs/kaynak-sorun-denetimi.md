...
| --- | --- | --- |
| Tarayıcıdan Discord web'e erişilememesi | Chrome, Edge, Firefox, Brave, Opera ve Vivaldi desteği kullanıcı açarsa kapsama alınır | Tarayıcı modu açıldığında Discord uygulaması ve bulunan desteklenen tarayıcı executable path'leri birlikte kapsama alınır; kapalıyken desteklenen tarayıcılar için geçici Discord web engeli uygulanır |
...
| Eski/şüpheli kurucu paketleri | WireSock resmi kurucusu çalışma zamanında doğrulanır | Kaynak repoya kurucu gömülmez; release arşivindeki fallback MSI varsa hash, imza, yayıncı ve sürümle doğrulanır |
...
| Şüpheli ikili dosya, VirusTotal ve kurucu güveni | `#80` | Kaynak repoya WireSock kurucusu, wgcf ikilisi, WinDivert veya DPI aracı eklenmez. Release arşivindeki WireSock fallback kurucusu varsa yalnızca SHA-256, Authenticode, yayıncı ve sürüm kontrolünden sonra kabul edilir. |
