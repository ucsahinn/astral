...
| --- | --- | --- |
| Tarayıcıdan Discord web'e erişilememesi | Chrome, Edge, Firefox, Brave, Opera ve Vivaldi süreçleri isteğe bağlı kapsama alındı | Discord web modu açıldığında desteklenen tarayıcılarla tünellenir |
...
| Discord dışı `Update.exe` kapsamı | Discord klasörleri, bilinen Discord exe adları ve web modu açıksa desteklenen tarayıcı adları kullanılır | Genel güncelleyici veya üçüncü taraf güncelleyici tünellenmez |
| Eski/şüpheli kurucu paketleri | WireSock resmi kurucusu çalışma zamanında doğrulanır | Repoya kurucu gömülmez |
| Profil bozulması veya geniş `AllowedApps` | Profil yeniden yazılır ve testlerle kilitlenir | Varsayılanda Discord uygulamaları kalır; web modu açıksa desteklenen tarayıcılar eklenir |
...
| Tarayıcıdan Discord web, otomatik sayfa veya tarayıcı tüneli davranışı | `#167`, `#127`, `#121`, `#97`, `#94` | Tarayıcı desteği opsiyonel ürün kararıdır ve varsayılan kapalıdır. Desteklenen tarayıcı süreçleri yalnızca web modu açıldığında tünellenir; kapatınca WireSock süreci sonlandırılır ve kalıcı tarayıcı/DNS/firewall kuralı bırakılmaz. |
